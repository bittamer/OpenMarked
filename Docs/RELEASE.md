...
```text
dist/OpenMarked-0.5.0-macOS.zip
dist/OpenMarked-0.5.0-macOS.dmg
```
...
- Artifact path: `dist/OpenMarked-0.5.0/OpenMarked.app`, `dist/OpenMarked-0.5.0-macOS.zip`, and `dist/OpenMarked-0.5.0-macOS.dmg`
...
- Owner approval: pending before tag and GitHub publication
- Release tag: `v0.5.0`

Use this command only after approval and after all release changes are committed:

```sh
git tag -a v0.5.0 -m "OpenMarked 0.5.0"
```
