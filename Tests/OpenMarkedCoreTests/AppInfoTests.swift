...
struct AppInfoTests {
    ...
    func versionIsCurrentReleaseVersion() {
        #expect(AppInfo.version == "0.5.2")
        #expect(AppInfo.build == "8")
    }
    ...
    @Test("Inspector state transitions preserve selected section")
    func inspectorStateTransitions() {
        var state = DocumentWindowState()

        #expect(!state.layout.isInspectorVisible)
        #expect(state.layout.selectedInspectorSection == .summary)

        state.toggleInspector()
        #expect(state.layout.isInspectorVisible)
        #expect(state.statusMessage == "Inspector shown")

        state.selectInspectorSection(.links)
        #expect(state.layout.selectedInspectorSection == .links)
        #expect(state.layout.isInspectorVisible)

        state.showInspector(section: .export)
        #expect(state.layout.isInspectorVisible)
        #expect(state.layout.selectedInspectorSection == .export)
    ...
    @Test("Window layout decodes old payloads with inspector defaults")
    func windowLayoutDecodesOldPayloadWithInspectorDefaults() throws {
        let data = Data(
            """
            {
        ...
        #expect(!layout.isOutlineVisible)
        #expect(!layout.isInspectorVisible)
        #expect(layout.selectedInspectorSection == .summary)
        #expect(layout.outlineDisplayOptions == .default)
        #expect(layout.selectedThemeID == "github")
        #expect(layout.fontScale == 1.2)
        ...
        #expect(unknownSectionLayout.isInspectorVisible)
        #expect(unknownSectionLayout.selectedInspectorSection == .summary)
    }
    ...
    @Test("Document inspection builds empty document without render result")
    func documentInspectionBuildsEmptyDocumentWithoutRenderResult() {
        let document = MarkdownDocument(
        ...
        #expect(report.metadata.displayTitle == "empty.md")
        #expect(report.metadata.titleSource == .fileName)
        #expect(report.statistics == RichDocumentStatistics.empty)
        #expect(report.links.isEmpty)
        #expect(report.assets.isEmpty)
        #expect(report.exportReadiness.isReady)
    }

    @Test("Document inspection builds metadata report without render result")
    func documentInspectionBuildsMetadataReportWithoutRenderResult() throws {
        ...
        #expect(report.metadata.displayTitle == "Workbench Metadata Fixture")
        #expect(report.metadata.titleSource == .frontMatter)
        #expect(report.metadata.frontMatterFormat == .yaml)
        #expect(report.metadata.fields.contains { $0.key == "tags" && $0.value == "inspection, metadata, release" && $0.valueKind == .list && $0.tokens == ["inspection", "metadata", "release"] })
        #expect(report.metadata.fields.contains { $0.key == "draft" && $0.valueKind == .boolean && $0.value == "false" })
        #expect(report.metadata.fields.contains { $0.key == "date" && $0.valueKind == .date })
        #expect(report.metadata.fields.contains { $0.key == "priority" && $0.valueKind == .number && $0.value == "3" })
        #expect(report.metadata.fields.contains { $0.key == "aliases" && $0.valueKind == .list && $0.tokens == ["Workbench", "Document Inspector"] })
        #expect(report.metadata.fields.contains { $0.key == "options" && $0.valueKind == .object && $0.value.contains("mode: compact") })
        #expect(report.metadata.fields.contains { $0.key == "custom-field" && !$0.isStandard })
        #expect(report.metadata.fileFacts.contains { $0.key == "titleSource" && $0.value == "Front matter" })
        #expect(report.metadata.fileFacts.contains { $0.key == "path" && $0.value.hasSuffix("metadata-rich.md") })
        #expect(report.statistics.words == document.statistics.wordCount)
        #expect(report.statistics.linkCount == 0)
    }

    @Test("JSON front matter is parsed for metadata inspection")
    func jsonFrontMatterIsParsedForMetadataInspection() throws {
        let url = URL(fileURLWithPath: "Fixtures/Markdown/json-front-matter.md").standardizedFileURL
        let document = try MarkdownDocumentLoader.load(url: url, createBookmark: false)
        let report = DocumentInspectionBuilder.build(document: document)

        #expect(document.frontMatter?.format == .json)
        #expect(document.resolvedTitle == "JSON Metadata Fixture")
        #expect(report.metadata.frontMatterFormat == .json)
        #expect(report.metadata.fields.contains { $0.key == "tags" && $0.valueKind == .list && $0.tokens == ["json", "metadata"] })
        #expect(report.metadata.fields.contains { $0.key == "draft" && $0.valueKind == .boolean && $0.value == "false" })
        #expect(report.metadata.fields.contains { $0.key == "priority" && $0.valueKind == .number && $0.value == "2" })
    }

    @Test("Malformed front matter produces diagnostics without blocking render")
    func malformedFrontMatterProducesDiagnosticsWithoutBlockingRender() throws {
        ...
        #expect(document.frontMatter?.title == "Malformed Metadata Fixture")
        #expect(!document.bodyText.contains("broken field without separator"))
        #expect(document.frontMatterDiagnostics.contains { $0.kind == .malformedFrontMatter && $0.source == "broken field without separator" })
        #expect(result.diagnostics.contains { $0.kind == .malformedFrontMatter && $0.source == "broken field without separator" })
        #expect(report.diagnostics.contains { $0.kind == .malformedFrontMatter })
        #expect(!report.exportReadiness.isReady)
        #expect(report.exportReadiness.issues.contains { $0.title == "Malformed front matter" })
    }

    @Test("Document inspection reports rich statistics")
    func documentInspectionReportsRichStatistics() throws {
        let url = URL(fileURLWithPath: "Fixtures/Markdown/statistics-rich.md").standardizedFileURL
        let document = try MarkdownDocumentLoader.load(url: url, createBookmark: false)
        let result = try CMarkGFMRenderer().render(RenderRequest(document: document))
        let report = DocumentInspectionBuilder.build(document: document, renderResult: result)

        #expect(report.statistics.headingCount >= 6)
        #expect(report.statistics.linkCount == 1)
        #expect(report.statistics.imageCount == 1)
        #expect(report.statistics.codeBlockCount == 2)
        #expect(report.statistics.tableCount == 1)
        #expect(report.statistics.calloutCount == 1)
        #expect(report.statistics.mermaidDiagramCount == 1)
        #expect(report.statistics.mathExpressionCount == 2)
        #expect(report.statistics.headingLevels[2] == 6)
        #expect(report.statistics.sectionStatistics.filter { $0.level == 2 }.count == 6)
        #expect(report.statistics.longestSection != nil)
        #expect(report.statistics.estimatedPageCount >= 1)
        #expect(report.exportReadiness.isReady)
    }

    @Test("Document inspection reports links, assets, and export readiness")
    func documentInspectionReportsLinksAssetsAndReadiness() throws {
        let url = URL(fileURLWithPath: "Fixtures/Markdown/inspection-links-assets.md").standardizedFileURL
        let document = try MarkdownDocumentLoader.load(url: url, createBookmark: false)
        let result = try CMarkGFMRenderer().render(RenderRequest(document: document))
        let report = DocumentInspectionBuilder.build(document: document, renderResult: result)

        #expect(report.metadata.displayTitle == "Inspection Links And Assets")
        #expect(report.metadata.frontMatterFormat == .toml)
        #expect(report.links.contains { $0.target == "README.md" && $0.status == .valid })
        #expect(report.links.contains { $0.target == "#asset-section" && $0.kind == .sameDocumentHeading && $0.status == .valid })
        #expect(report.links.contains { $0.target == "missing-guide.md" && $0.status == .missing })
        #expect(report.links.contains { $0.target == "https://example.com/openmarked" && $0.kind == .remoteURL && $0.status == .skipped })
        #expect(report.links.contains { $0.target == "https://" && $0.status == .malformed })
        #expect(report.links.contains { $0.target == "javascript:alert" && $0.status == .unsupported })
        #expect(report.assets.contains { $0.source == "../Assets/sample-mark.svg" && $0.status == .valid })
        #expect(report.assets.contains { $0.source == "../Assets/missing-image.png" && $0.status == .missing })
        #expect(report.assets.contains { $0.source == "https://example.com/openmarked.png" && $0.kind == .remoteImage && $0.status == .skipped })
        #expect(report.links.contains { $0.target == "README.md" && $0.resolvedPath?.hasSuffix("Fixtures/Markdown/README.md") == true })

        guard let localAsset = report.assets.first(where: { $0.source == "../Assets/sample-mark.svg" }) else {
            Issue.record("Expected local sample asset in inspection report")
            return
        }
        #expect(localAsset.resolvedPath?.hasSuffix("Fixtures/Assets/sample-mark.svg") == true)
        #expect((localAsset.fileInfo?.byteSize ?? 0) > 0)
        #expect(localAsset.fileInfo?.pixelWidth == 640)
        #expect(localAsset.fileInfo?.pixelHeight == 360)
        #expect(!report.exportReadiness.isReady)
        #expect(report.exportReadiness.issues.contains { $0.title == "Missing local link" && $0.source == "missing-guide.md" })
        #expect(report.exportReadiness.issues.contains { $0.title == "Missing image" && $0.source == "../Assets/missing-image.png" })
        #expect(report.exportReadiness.issues.contains { $0.title == "Remote image" && $0.source == "https://example.com/openmarked.png" })
    }

    @Test("Export readiness reports blocked remote images")
    func exportReadinessReportsBlockedRemoteImages() throws {
        ...
        #expect(report.assets.contains { $0.source == "https://example.com/image.png" && $0.status == .blocked })
        #expect(!report.exportReadiness.isReady)
        #expect(report.exportReadiness.issues.contains { $0.title == "Remote image blocked" && $0.source == "https://example.com/image.png" })
    }
    ...
    @Test("Title falls back to first heading for preview and export")
    func titleFallsBackToFirstHeadingForPreviewAndExport() throws {
        let url = URL(fileURLWithPath: "Fixtures/Markdown/readme.md").standardizedFileURL
        let document = try MarkdownDocumentLoader.load(url: url, createBookmark: false)
        let result = try CMarkGFMRenderer().render(RenderRequest(document: document))
        ...
        #expect(document.displayTitle == "readme.md")
        #expect(document.resolvedTitle == "OpenMarked Fixture README")
        #expect(document.resolvedTitleSource == .firstHeading)
        #expect(result.fullHTML.contains("<title>OpenMarked Fixture README</title>"))
        #expect(exportedHTML.contains("<title>OpenMarked Fixture README</title>"))
        #expect(report.metadata.displayTitle == "OpenMarked Fixture README")
        #expect(report.metadata.titleSource == .firstHeading)
    }

    @Test("Unsupported files are rejected")
    func unsupportedFilesAreRejected() {
        do {
            _ = try MarkdownDocumentLoader.load(url: URL(fileURLWithPath: "Package.swift"))
            Issue.record("Expected unsupported Package.swift file to be rejected")
        } catch {
            #expect((error as? DocumentOpenError)?.kind == .unsupportedFileType)
        }
    }

    @Test("Window state store persists layout and export destinations")
    func windowStateStorePersistsLayout() throws {
        let suiteName = "OpenMarkedTests-\(UUID().uuidString)"
        guard let userDefaults = UserDefaults(suiteName: suiteName) else {
            Issue.record("Expected test user defaults suite")
            return
        }
        defer { userDefaults.removePersistentDomain(forName: suiteName) }
        ...
        #expect(restored?.layout.outlineDisplayOptions.mode == .flat)
        #expect(restored?.layout.outlineDisplayOptions.maximumVisibleLevel == 3)
        #expect(restored?.layout.outlineDisplayOptions.showsAutoNumbers == true)
        #expect(restored?.frame?.width == 900)
        #expect(restored?.exportDestinations == exportDestinations)
    }

    @Test("Window state store preserves multiple documents when updating one")
    func windowStateStorePreservesMultipleDocumentsWhenUpdatingOne() throws {
        ...
        #expect(store.restore(forDocumentID: first.documentID) == updatedFirst)
        #expect(store.restore(forDocumentID: second.documentID) == second)
        #expect(store.loadAll().count == 2)

        let reloadedStore = DocumentWindowStateStore(userDefaults: userDefaults, storageKey: storageKey)
        #expect(reloadedStore.restore(forDocumentID: first.documentID) == updatedFirst)
        #expect(reloadedStore.restore(forDocumentID: second.documentID) == second)
    }

    @Test("Window state store falls back from corrupted persisted data")
    func windowStateStoreFallsBackFromCorruptedPersistedData() throws {
        ...
        #expect(store.restore(forDocumentID: "/tmp/missing.md") == nil)

        let replacement = persistedWindowState(documentID: "/tmp/recovered.md", themeID: "default", width: 700, savedAt: 4)
        store.save(replacement)

        let reloadedStore = DocumentWindowStateStore(userDefaults: userDefaults, storageKey: storageKey)
        #expect(reloadedStore.loadAll() == [replacement.documentID: replacement])
    }

    @Test("Window state store does not reload defaults after initialization")
    func windowStateStoreDoesNotReloadDefaultsAfterInitialization() throws {
        let suiteName = "OpenMarkedWindowStateReadCountTests-\(UUID().uuidString)"
        guard let userDefaults = CountingUserDefaults(suiteName: suiteName) else {
            Issue.record("Expected counting user defaults suite")
            return
        }
        defer { userDefaults.removePersistentDomain(forName: suiteName) }
        ...
        #expect(userDefaults.dataReadCount == 1)
    ...
    @Test("Heading slugs are deduplicated")
    func headingSlugsAreDeduplicated() {
        let processed = HeadingPostProcessor.process("<h2>Repeat</h2>\n<h2>Repeat</h2>")

        #expect(processed.html.contains(#"id="repeat""#))
        #expect(processed.html.contains(#"id="repeat-1""#))
        #expect(processed.outline.map(\.id) == ["repeat", "repeat-1"])
    }
    ...
    @Test("Missing local image produces a diagnostic")
    func missingLocalImageDiagnostic() throws {
        let url = URL(fileURLWithPath: NSTemporaryDirectory())
    ...
    func footnotesRenderWhenEnabled() throws {
        let url = URL(fileURLWithPath: "Fixtures/Markdown/footnotes.md").standardizedFileURL
        let document = try MarkdownDocumentLoader.load(url: url, createBookmark: false)
        let result = try CMarkGFMRenderer().render(RenderRequest(document: document))

        #expect(result.bodyHTML.contains("footnote"))
    }
    ...
    @Test("Preview HTML security policy removes scripts and event handlers")
    func previewHTMLSecurityPolicyRemovesScriptsAndEventHandlers() {
        let unsafeHTML = #"""
        ...
        #expect(!lowercasedHTML.contains("<script"))
        #expect(!lowercasedHTML.contains("onclick"))
        #expect(!lowercasedHTML.contains("onmouseover"))
        #expect(!lowercasedHTML.contains("onfocus"))
        #expect(!lowercasedHTML.contains("javascript:"))
        #expect(!lowercasedHTML.contains("java&#10;script:"))
        #expect(!lowercasedHTML.contains("java&tab;script:"))
        #expect(!lowercasedHTML.contains("vbscript:"))
        #expect(!lowercasedHTML.contains("data:text/html"))
        #expect(sanitizedHTML.contains(#"href="https://example.com/#safe""#))
        #expect(sanitizedHTML.contains("href=\"#fragment\""))
        #expect(sanitizedHTML.contains(#"href="notes/page.md""#))
        #expect(sanitizedHTML.contains(#"href="mailto:team@example.com""#))
        #expect(sanitizedHTML.contains(#"href="file:///tmp/openmarked.md""#))
        #expect(sanitizedHTML.contains(#"src="data:image/png;base64,AAAA""#))
    }
